...
---nas同步到s3------
# 同步基础模型---这个exclude的格式和rsync不太一样
nohup aws s3 sync --exclude "changjinglsx/*" --exclude "giftbox/*" --exclude "Giftbox11/*"  --exclude "SD_xihua*"  /mnt/sdwebui_public/public/models/Stable-diffusion  s3://sd-web-ui-ec2/stable-diffusion-webui/models/Stable-diffusion-Clean --no-progress > aws_sync.log &
tail -f aws_sync.log
rm -rf aws_sync.log
...
-----阿里云云函数不走企业微信的nginx的配置-----
笔记搜索[阿里云函数nginx配置]即可
